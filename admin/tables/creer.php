...
requirePermission('tables.creer');
...
if ($evenement_id > 0) {
    try {
        $stmt = $pdo->prepare("SELECT id, nom FROM evenements WHERE id = ? AND statut != 'ANNULE'");
        $stmt->execute([$evenement_id]);
        $evenement = $stmt->fetch(PDO::FETCH_ASSOC);
    } catch (PDOException $e) {
        error_log('Erreur récupération événement (tables/creer) : ' . $e->getMessage());
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("SELECT id FROM tables WHERE id_evenement = ? AND nom = ?");
            $stmt->execute([$evenement_id, $nom]);
            if ($stmt->fetch()) {
                $errors[] = "Une table avec ce nom existe déjà pour cet événement.";
            }
        } catch (PDOException $e) {
            error_log('Erreur vérification unicité table : ' . $e->getMessage());
    ...
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("
                INSERT INTO tables (
                    id_evenement, nom, numero, capacite_min, capacite_max,
                    type, zone, position_x, position_y, actif, created_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, NOW())
            ");
            $stmt->execute([
                $evenement_id,
            ...
            if (function_exists('logAction')) {
                logAction(
                    ...
                    'CREATE_TABLE',
                    'tables',
                    "Création de la table '$nom' pour l'événement '{$evenement['nom']}' (ID: $tableId)"
                );
            }

            header('Location: index.php?evenement=' . $evenement_id . '&success=ajoute');
            exit;

        } catch (PDOException $e) {
            error_log('Erreur création table : ' . $e->getMessage());
            $error = 'Erreur lors de la création de la table. Veuillez réessayer.';
        }
    } else {
        $error = implode("\n", $errors);
    }
...
<head>
    ...
    <title>Ajouter une table - <?php echo APP_NAME; ?></title>
    ...
    <div class="main-content" id="mainContent">
        ...
        <div class="top-bar">
            <div class="page-title">
                <h4><i class="bi bi-plus-circle-fill"></i> Ajouter une table</h4>
                <small><i class="bi bi-table"></i> Pour l'événement : <?php echo htmlspecialchars($evenement['nom']); ?></small>
            </div>
            <div class="user-info">
        ...
        <div class="content-section">
            <div class="form-container fade-in">
                ...
                <h5 class="form-title"><i class="bi bi-plus-circle-fill"></i> Nouvelle table</h5>
                ...
                <div class="event-info-container">
                    ...
                    <div class="event-text">
                        <small>Événement sélectionné</small>
                ...
                <form method="POST" action="">
                    <div class="row">
                        <div class="col-md-8 mb-3">
                            ...
                            <input type="text" class="form-control" name="nom" value="<?php echo htmlspecialchars($nom); ?>"
                                   placeholder="Ex: Table 1, VIP 1..." required maxlength="100">
                        </div>
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="text" class="form-control" name="numero" value="<?php echo htmlspecialchars($numero); ?>"
                                   placeholder="Ex: 01, A1..." maxlength="20">
                        </div>
                    </div>
                    ...
                    <div class="d-flex gap-3 mt-4 flex-wrap">
                        ...
                        <a href="index.php?evenement=<?php echo $evenement_id; ?>" class="btn btn-cancel">
