...
if (!isAdmin()) {
    ...
    die('<div style="padding:50px;text-align:center;font-family:sans-serif;background:#f8f5f2;min-height:100vh;display:flex;flex-direction:column;justify-content:center;align-items:center;">
        ...
        <p style="color:#6a5a4a;margin-bottom:30px;">Seul un administrateur peut modifier une boisson du catalogue.</p>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("SELECT id FROM boissons WHERE nom = ? AND id != ?");
            $stmt->execute([$nom, $id]);
            if ($stmt->fetch()) {
    ...
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("
                UPDATE boissons 
                SET nom = ?, description = ?, type = ?, actif = ?
                WHERE id = ?
            ");
            $stmt->execute([$nom, $description, $type, $actif, $id]);

            if (function_exists('logAction')) {
                logAction($user['id'], 'UPDATE_DRINK', 'boissons', 
                          "Modification de la boisson '$nom' (ID: $id)");
            }
...
<head>
    ...
    <title>Modifier une boisson - <?php echo APP_NAME; ?></title>
    ...
    <div class="main-content" id="mainContent">
        ...
        <div class="top-bar">
            <div class="page-title">
                <h4><i class="bi bi-pencil-fill"></i> Modifier la boisson</h4>
                <small><i class="bi bi-cup-straw"></i> Modifier les informations du catalogue</small>
            </div>
            <div class="user-info">
        ...
        <div class="content-section">
            <div class="form-container fade-in">
                ...
                <h5 class="form-title">
                    ...
                    Modifier - <?php echo htmlspecialchars($boisson['nom']); ?>
                </h5>
                ...
                <div class="user-info-banner">
                    ...
                    <div>
                        Cette boisson fait partie du <strong>catalogue global</strong>. 
                        Modifier son nom ou son type affectera tous les événements qui l'utilisent.
                    </div>
                </div>
                ...
                <form method="POST" action="">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-text">
                            Les boissons inactives ne pourront pas être associées aux événements.
                            <?php if (!$actif): ?>
                                <br><strong style="color: #dc2626;">⚠️ Cette boisson est actuellement inactive.</strong>
                            <?php endif; ?>
                        </div>
                    </div>
                    ...
                    <div class="d-flex gap-3 mt-4 flex-wrap">
                        ...
                        <?php if (hasPermission('boissons.modifier')): ?>
                            <a href="associer.php?id=<?php echo $id; ?>" class="btn-cancel">
                                <i class="bi bi-link"></i> Associer aux événements
                            </a>
                        <?php endif; ?>
                    </div>
                </form>
