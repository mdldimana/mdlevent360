...
if ($evenementId > 0) {
    try {
        ...
        error_log('Erreur événement export boissons : ' . $e->getMessage());
    }
    if (!$evenement) die('Erreur : événement introuvable.');
}

// ========== RÉCUPÉRATION DES BOISSONS CHOISIES ==========
$boissons = [];
try {
    if ($evenementId > 0) {
        $stmt = $pdo->prepare("
    ...
    error_log('Erreur récupération boissons export : ' . $e->getMessage());
}

// ========== STATS (pour l'en-tête uniquement) ==========
$totalBoissonsDistinctes = count($boissons);
$totalQuantiteGlobale    = 0;
$totalInvitesConcernes   = 0;

foreach ($boissons as $b) {
    $totalQuantiteGlobale += (int)($b['total_quantite'] ?? 0);
}
// Nombre d'invités uniques ayant choisi au moins une boisson
try {
    if ($evenementId > 0) {
        $stmt = $pdo->prepare("
            SELECT COUNT(DISTINCT pi.id_invitation) AS nb 
            FROM preferences_invitation pi
            INNER JOIN invitations i ON i.id = pi.id_invitation
            WHERE i.id_evenement = ?
        ");
        $stmt->execute([$evenementId]);
        $totalInvitesConcernes = (int)($stmt->fetch()['nb'] ?? 0);
    }
} catch (PDOException $e) {
    error_log('Erreur comptage invités export boissons : ' . $e->getMessage());
}
...
// ========== MÉTADONNÉES ==========
$reference      = 'REF-BOIS-' . strtoupper(substr(md5($evenementId . '-' . date('Ymd')), 0, 8));
...
$nomFichierPdf = 'Boissons_' . preg_replace('/[^A-Za-z0-9_-]/', '-', $nomEvenement) . '_' . date('Y-m-d') . '.pdf';
?>
<!DOCTYPE html>
<html lang="fr">
<head>
...
<title>Détail des boissons - <?php echo htmlspecialchars($nomEvenement); ?> - <?php echo APP_NAME; ?></title>
...
<style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .pdf-table .boisson-desc {
        font-size: 11px; color: #9a8a7f; margin-top: 2px;
        font-style: italic; line-height: 1.4;
    }
    .pdf-table .invite-sub {
        font-size: 11px; color: #9a8a7f; margin-top: 2px;
    }
...
<div class="pdf-document" id="pdfContent">
    ...
    <div class="pdf-header">
        ...
        <div class="pdf-title-section">
            <div class="pdf-doc-type">🍷 Détail des boissons</div>
            ...
            <div class="event-info">
                ...
                <span><i class="bi bi-cup-hot-fill"></i> <?php echo $totalBoissonsDistinctes; ?> boisson(s)</span>
                <span><i class="bi bi-basket-fill"></i> <?php echo $totalQuantiteGlobale; ?> quantité(s) demandée(s)</span>
            </div>
        </div>
    </div>
                ...
                <div class="pdf-section-header">
                    ...
                    <div>
                        ...
                        <div class="pdf-section-subtitle">
                            <?php echo $totalBoissonsDistinctes; ?> boisson(s) — 
                            <?php echo $totalQuantiteGlobale; ?> quantité(s) totale(s)
                        </div>
                    </div>
                </div>
                        ...
                        <?php $i = 1; foreach ($boissons as $b): ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <?php if (!empty($b['boisson_description'])): ?>
                                        <div class="boisson-desc">
                                            <?php echo htmlspecialchars(mb_strimwidth($b['boisson_description'], 0, 80, '...')); ?>
                                        </div>
                                    <?php endif; ?>
                                </td>
                                <td>
        ...
        <?php else: ?>
            ...
            <div class="pdf-empty">
                ...
                <div class="pdf-empty-text">
                    Aucun invité n'a encore choisi de boisson pour cet événement.
                </div>
            </div>
...
console.log('📄 Détail des boissons - Référence: <?php echo $reference; ?>');
</script>
