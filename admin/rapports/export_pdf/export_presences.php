...
if ($evenementId > 0) {
    try {
        ...
        error_log('Erreur événement export présences : ' . $e->getMessage());
    }
...
try {
    if ($evenementId > 0) {
        $stmt = $pdo->prepare("
            SELECT 
                ...
                p.date_entree,
                p.heure_entree,
                p.created_at AS presence_created_at,
    ...
    error_log('Erreur récupération présences export : ' . $e->getMessage());
}

// ========== STATS (pour l'en-tête uniquement) ==========
$totalPresences = count($presences);
$totalPersonnesEntrees = 0;

foreach ($presences as $p) {
    $totalPersonnesEntrees += (int)($p['nombre_present'] ?? 1);
}

// ========== MÉTADONNÉES ==========
$reference      = 'REF-PRES-' . strtoupper(substr(md5($evenementId . '-' . date('Ymd')), 0, 8));
...
$nomFichierPdf = 'Presences_' . preg_replace('/[^A-Za-z0-9_-]/', '-', $nomEvenement) . '_' . date('Y-m-d') . '.pdf';
?>
<!DOCTYPE html>
<html lang="fr">
<head>
...
<title>Liste des présences - <?php echo htmlspecialchars($nomEvenement); ?> - <?php echo APP_NAME; ?></title>
...
<div class="pdf-document" id="pdfContent">
    ...
    <div class="pdf-header">
        ...
        <div class="pdf-title-section">
            <div class="pdf-doc-type">🎯 Liste des présences</div>
            ...
            <div class="event-info">
                ...
                <span><i class="bi bi-clipboard-check-fill"></i> <?php echo $totalPresences; ?> entrée(s)</span>
                <span><i class="bi bi-people-fill"></i> <?php echo $totalPersonnesEntrees; ?> personne(s) présente(s)</span>
            </div>
        </div>
    </div>
        ...
        <?php if (!empty($presences)): ?>
            ...
            <div class="pdf-section">
                <div class="pdf-section-header">
                    ...
                    <div>
                        <div class="pdf-section-title">Détail des entrées</div>
                        <div class="pdf-section-subtitle">
                            <?php echo $totalPresences; ?> entrée(s) — 
                            <?php echo $totalPersonnesEntrees; ?> personne(s)
                        </div>
                    </div>
                </div>
                ...
                <table class="pdf-table">
                    <thead>
                        <tr>
                            ...
                            <th style="width:17%; text-align:center;">Entrée</th>
                    ...
                    <tbody>
                        <?php $i = 1; foreach ($presences as $p): ?>
                            ...
                            $tsEntree = !empty($horodatage) ? strtotime($horodatage) : null;
                                ...
                                <td>
                                    ...
                                    <?php if (!empty($p['telephone'])): ?>
                                        <div class="invite-sub">
                                            <i class="bi bi-telephone-fill"></i> <?php echo htmlspecialchars($p['telephone']); ?>
                                        </div>
                                    <?php endif; ?>
                                </td>
                                <td>
                                ...
                                <td class="col-center">
                                    <?php if ($tsEntree): ?>
                                        <span class="time-badge">
                                            ...
                                            <?php echo date('H:i', $tsEntree); ?>
                                        </span>
                                        <div style="font-size:10px; color:#9a8a7f; margin-top:2px;">
                                            <?php echo date('d/m/Y', $tsEntree); ?>
                                        </div>
                                    <?php else: ?>
                                        <span style="color:#b8a99c; font-style:italic; font-size:11px;">—</span>
                                    <?php endif; ?>
        ...
        <?php else: ?>
            ...
            <div class="pdf-empty">
                ...
                <div class="pdf-empty-text">
                    Aucune entrée n'a encore été enregistrée pour cet événement.
                </div>
            </div>
...
console.log('📄 Liste des présences - Référence: <?php echo $reference; ?>');
</script>
