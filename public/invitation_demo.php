<?php
// ============================================================
// INVITATION DE DÉMONSTRATION - MdlEvent
// ============================================================
...
// ============================================================
// Cette détection fonctionne en local ET en ligne

// Méthode 1 : Détecter via le script en cours
...
// Si le chemin est vide ou juste '/', c'est la racine
...
// echo "<!-- Image path: " . $imageBasePath . " -->\n";
// echo "<!-- DOCUMENT_ROOT: " . $_SERVER['DOCUMENT_ROOT'] . " -->\n";
// echo "<!-- SCRIPT_NAME: " . $scriptPath . " -->\n";

// ============================================================
// DONNÉES DE DÉMONSTRATION
// ============================================================
$demoData = [
    'code_unique' => 'MDL-DEMO-2024',
    'evenement_nom' => 'Olivia & Trésor',
    'type_evenement' => 'Mariage',
    'date_evenement' => '2024-12-15',
    'heure_evenement' => '18:00:00',
    'lieu' => 'Salle des Fêtes Exodus',
    'adresse' => 'Av Victoire Matonge Kalamu, Kinshasa',
    'description' => "C'est avec une immense joie que Olivia & Trésor vous convient à célébrer leur mariage.\n\nCet événement si spécial marquera l'union de deux âmes qui se sont trouvées et qui souhaitent partager leur bonheur avec vous.\n\nVotre présence est notre plus beau cadeau.",
    'fond' => '',
    'invite_nom' => 'Mavakala',
    'invite_prenom' => 'Gradi',
    'invite_email' => 'gradi.mavakala@email.com',
    'invite_telephone' => '+243 812 345 678',
    'nb_places_max' => 2,
    'statut' => 'EN_ATTENTE',
    'reponse' => null,
    'nb_confirme' => null,
...
$photoTitles = ['Moment de joie', 'Cérémonie', 'Réception', 'Couple', 'Fête'];
$photoDescs = ['Les mariés rayonnants', 'L\'échange des vœux', 'La fête commence', 'Les amoureux', 'La célébration'];

$demoPhotos = [];
foreach ($photosList as $index => $filename) {
    $demoPhotos[] = [
        'id' => $index + 1,
        'photo' => $imageBasePath . $filename,
        'titre' => $photoTitles[$index] ?? 'Photo',
        'description' => $photoDescs[$index] ?? ''
    ];
}

// Image de fond
$fondImage = $imageBasePath . 'fond.jpeg';
...
function imageExists($path) {
    ...
    // Chemin alternatif (sans le sous-dossier)
    $altPath = preg_replace('#/[^/]+/#', '/', $fullPath, 1);
    if (file_exists($altPath)) return true;
    
    // Chemin relatif
    if (file_exists(__DIR__ . '/..' . $path)) return true;
...
// Vérifier les images
$hasFond = imageExists($fondImage);
$hasPhotos = false;
foreach ($demoPhotos as $photo) {
    if (imageExists($photo['photo'])) {
        $hasPhotos = true;
        break;
    }
}
...
// Boissons
$demoBoissons = [
    ['id' => 1, 'nom' => 'Champagne', 'type' => 'Alcool', 'description' => 'Moelleux et pétillant'],
    ['id' => 2, 'nom' => 'Vin Rouge', 'type' => 'Alcool', 'description' => 'Corsé et fruité'],
    ['id' => 3, 'nom' => 'Vin Blanc', 'type' => 'Alcool', 'description' => 'Sec et frais'],
    ['id' => 4, 'nom' => 'Coca-Cola', 'type' => 'Soft', 'description' => 'Classique'],
    ['id' => 5, 'nom' => 'Fanta', 'type' => 'Soft', 'description' => 'Orange'],
    ['id' => 6, 'nom' => 'Jus de fruit', 'type' => 'Soft', 'description' => 'Multivitaminé'],
    ['id' => 7, 'nom' => 'Eau plate', 'type' => 'Eau', 'description' => 'Naturelle'],
    ['id' => 8, 'nom' => 'Eau gazeuse', 'type' => 'Eau', 'description' => 'Pétillante'],
];

// URL de l'invitation
$protocol = isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' ? 'https://' : 'http://';
$host = $_SERVER['HTTP_HOST'] ?? 'localhost';
$invitationUrl = $protocol . $host . $basePath . '/public/invitation.php?code=MDL-DEMO-2024';

// QR Code
$qrCodeUrl = 'https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=' . urlencode($invitationUrl) . '&color=1a1a2e&bgcolor=ffffff&qzone=1';

// Icône boisson
function getBoissonIcon($type) {
...
// Grouper les boissons
$boissonsGrouped = [];
foreach ($demoBoissons as $b) {
    $type = $b['type'] ?? 'Autres';
...
<head>
    ...
    <title>Invitation - <?php echo htmlspecialchars($demoData['evenement_nom']); ?></title>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        body {
            ...
            <?php if ($hasFond): ?>
            background-image: url('<?php echo htmlspecialchars(getImageUrl($fondImage)); ?>');
        ...
        .separator {
            width: 60px; height: 1px; background: rgba(234, 227, 220, 0.6); 
        ...
        @media (max-width: 768px) {
            ...
            .diaporama-container { aspect-ratio: 16/9; }
...
<div class="app-wrapper">
    ...
    <div id="invitation-card" class="section-animee zoom-in delai-1">
        ...
        <div class="monogram-bg" id="monogramDisplay">
            <?php echo htmlspecialchars($demoData['evenement_nom'][0]); ?>
        </div>
        ...
        <div class="card-content">
            ...
            <div style="margin-top:40px;">
                ...
                <div style="font-family:'Inter',sans-serif;font-size:15px;letter-spacing:0.18em;text-transform:uppercase;font-weight:600;color:#1a1a1a;">
                    <?php echo htmlspecialchars($demoData['invite_prenom'] . ' ' . $demoData['invite_nom']); ?>
                </div>
            ...
            <div style="margin-top:40px;">
                <h1 style="font-family:'Great Vibes', cursive;line-height:0.88;letter-spacing:-0.03em;color:#1a1a1a;font-weight:400;">
                    <span style="display:block;font-size:4.5rem;">
                        <?php echo htmlspecialchars($demoData['evenement_nom']); ?>
                    </span>
                </h1>
                ...
                <div style="margin-top:8px;font-family:'Inter',sans-serif;font-size:12px;letter-spacing:0.16em;text-transform:uppercase;font-weight:500;color:#1a1a1a;">
                    célébrer leur <?php echo htmlspecialchars($demoData['type_evenement']); ?>
                </div>
            </div>
            ...
            <div style="margin-top:40px;display:flex;flex-wrap:wrap;align-items:center;justify-content:center;gap:12px;">
                <div class="badge-dark">
                    ...
                    <?php echo date('d/m/Y', strtotime($demoData['date_evenement'])); ?>
                </div>
                <div style="font-family:'Inter',sans-serif;font-size:11px;letter-spacing:0.12em;text-transform:uppercase;color:#8c7e73;">
                    à <?php echo date('H:i', strtotime($demoData['heure_evenement'])); ?> • <?php echo htmlspecialchars($demoData['type_evenement']); ?>
                </div>
            </div>
                    ...
                    <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;justify-content:center;">
                        ...
                        <div style="text-align:left;min-width:0;">
                            ...
                            <div style="font-family:'Inter',sans-serif;font-size:13px;line-height:1.5;color:#1a1a1a;word-wrap:break-word;">
                                <?php echo htmlspecialchars($demoData['lieu']); ?>
                                <?php if ($demoData['adresse']): ?>
                                    <span style="color:#9a8a7f;font-size:12px;"> — <?php echo htmlspecialchars($demoData['adresse']); ?></span>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
                    <a href="https://www.google.com/maps/search/?api=1&query=<?php echo urlencode($demoData['lieu'] . ' ' . $demoData['adresse']); ?>" target="_blank" style="display:inline-flex;align-items:center;gap:8px;padding:8px 20px;border-radius:9999px;background:rgba(193,124,96,0.9);color:white;font-family:'Inter',sans-serif;font-size:13px;font-weight:500;border:none;cursor:pointer;transition:all 0.3s ease;text-decoration:none;backdrop-filter:blur(4px);-webkit-backdrop-filter:blur(4px);margin-left:0;">
            ...
            <div style="display:flex;justify-content:space-between;align-items:flex-end;flex-wrap:wrap;gap:16px;margin-top:16px;">
                <div style="text-align:left;">
                    ...
                    <div style="font-family:'Great Vibes', cursive;font-size:32px;line-height:1;color:#1a1a1a;">
                        <?php echo htmlspecialchars(explode(' et ', $demoData['evenement_nom'])[0]); ?>
                    </div>
    ...
    <div class="section-animee fade-right delai-2" id="section-photos">
        ...
        <?php if ($hasPhotos): ?>
            <div class="diaporama-container" id="diaporama">
                ...
                ?>
                    <div class="slide <?php echo $photoIndex === 0 ? 'active' : ''; ?>" data-index="<?php echo $photoIndex; ?>">
                        <img src="<?php echo htmlspecialchars(getImageUrl($photo['photo'])); ?>" 
                             alt="<?php echo htmlspecialchars($photo['titre'] ?? 'Photo'); ?>"
                             loading="lazy"
            ...
            <?php endif; ?>
        <?php else: ?>
            <div style="text-align:center; padding:60px 24px; color:#9a8a7f; font-size:16px;">
                <i class="fas fa-image" style="font-size:50px; display:block; margin-bottom:16px; color:#d4c5b2;"></i>
                <p>Aucune photo trouvée</p>
                <p style="font-size:12px;margin-top:8px;color:#b8a99c;">
                    Chemin utilisé : <strong><?php echo htmlspecialchars($imageBasePath); ?></strong>
                </p>
            </div>
        <?php endif; ?>
    </div>
    ...
    <div class="section section-animee flip delai-3" id="section-confirmation">
        ...
        <div style="text-align:center; margin-bottom:24px;">
            <div style="font-family:'Inter',sans-serif;font-size:14px;color:#6a5a4a;">
                ...
                <strong><?php echo htmlspecialchars($demoData['invite_prenom'] . ' ' . $demoData['invite_nom']); ?></strong>
            </div>
            <div style="font-family:'Inter',sans-serif;font-size:12px;color:#b8a99c;margin-top:4px;">
                <i class="fas fa-calendar-alt" style="margin-right:6px;"></i>
                <?php echo date('d/m/Y', strtotime($demoData['date_evenement'])); ?>
                <?php if ($demoData['heure_evenement']): ?> • <?php echo date('H:i', strtotime($demoData['heure_evenement'])); ?><?php endif; ?>
            </div>
        </div>

        <form method="POST" action="#" onsubmit="event.preventDefault(); alert('Merci pour votre confirmation ! Vous recevrez une notification.');">
            <div class="form-group">
                <label><i class="fas fa-users"></i> Nombre de personnes</label>
                <input type="number" name="nombre_personnes" value="1" min="1" max="<?php echo $demoData['nb_places_max']; ?>">
                <div style="font-family:'Inter',sans-serif;font-size:12px;color:#b8a99c;margin-top:4px;">
                    <i class="fas fa-info-circle"></i> Max: <?php echo $demoData['nb_places_max']; ?> personne(s)
                </div>
            </div>
    ...
    <div class="section section-animee fade-left delai-4" id="section-boissons">
        <div class="section-title">
            ...
            <span style="display:inline-block;background:rgba(233,236,239,0.8);color:#6c757d;padding:4px 14px;border-radius:100px;font-size:12px;font-weight:600;margin-left:8px;">
                <i class="fas fa-lock"></i> Démo
            </span>
        </div>
        ...
        <div style="text-align:center;margin-bottom:16px;color:#6a5a4a;font-size:13px;">
            ...
            Sélectionnez 2 boissons (cliquez sur une boisson pour la sélectionner)
        </div>
        
        <form method="POST" action="#" onsubmit="event.preventDefault(); alert('Vos préférences ont été enregistrées ! 🍹');">
            ...
            <div class="text-muted small mt-2 text-center" style="font-family:'Inter',sans-serif;font-size:12px;color:#b8a99c;">
    ...
    <div class="section section-animee fade-right delai-3" id="section-qr">
        ...
        <div class="qr-wrapper" style="display:flex;flex-direction:column;align-items:center;gap:12px;padding:16px 0 8px;">
            ...
            <div style="font-family:'Inter',sans-serif;font-size:12px;color:#b8a99c;margin-top:4px;">Code: <?php echo htmlspecialchars($demoData['code_unique']); ?></div>
        </div>
    </div>
    ...
    if (typeof QRCode !== 'undefined') {
        try {
            new QRCode(document.getElementById('card-qrcode'), {
                text: '<?php echo addslashes($invitationUrl); ?>',
                width: 150,
                height: 150,
                colorDark: '#1a1a1a',
                colorLight: '#ffffff',
...
async function telechargerJPEG() {
    ...
    try {
        ...
        const name = '<?php echo htmlspecialchars($demoData['evenement_nom']); ?>';
